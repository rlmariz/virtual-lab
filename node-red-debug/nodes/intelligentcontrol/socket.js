...
function init(server, app, log, redSettings) {
    var plotSettings = redSettings.plot || {};

    if ((plotSettings.hasOwnProperty("path")) && (typeof plotSettings.path === "string")) {
        settings.path = plotSettings.path;
    }
    else { settings.path = 'plot'; }
    ...
    if (plotSettings.middleware) {
        if (typeof plotSettings.middleware === "function" || Array.isArray(plotSettings.middleware)) {
            dashboardMiddleware = plotSettings.middleware;
        }
    }

    log.info(`plot fullpth: ${fullPath}`);
    log.info(`__dirname: ${__dirname}`);

    fs.stat(path.join(__dirname, 'www/index.html'), function (err, stat) {
        app.use(join(settings.path, "manifest.json"), function (req, res) { res.send(mani); });
        app.use(join(settings.path), dashboardMiddleware, serveStatic(path.join(__dirname, "www")));
    });

    log.info(`Plot started at: ${fullPath}`);

    if (typeof plotSettings.ioMiddleware === "function") {
        io.use(plotSettings.ioMiddleware);
        log.info("****** 1 *********")
    } else if (Array.isArray(plotSettings.ioMiddleware)) {
        log.info("****** 2 *********")
        plotSettings.ioMiddleware.forEach(function (ioMiddleware) {
            io.use(ioMiddleware);
        });
    } else {
        log.info("****** 3 *********")
    ...
    io.on('connection', function (socket) {
        ...
        log.info(`io connection: ${socket.id}`);
